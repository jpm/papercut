...
# $Id: mysql.py,v 1.1 2002-01-10 16:21:00 jpm Exp $
import MySQLdb
import time
...
class Papercut_Backend:
    ...
    def get_BODY(self, group_name, id):
        ...
        stmt = """
                ...
                WHERE
                    id=%s""" % (table_name, id)
        self.cursor.execute(stmt)
        return mime_encode_header(self.cursor.fetchone()[0])
    ...
    def get_XOVER(self, group_name, start_id, end_id='ggg'):
        ...
        for row in result:
            ...
            overviews.append("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (row[0], mime_encode_header(row[4]), mime_encode_header(author), formatted_time, message_id, row[1], len(row[6]), line_count, xref))
    ...
    def get_XPAT(self, group_name, header, pattern, start_id, end_id='ggg'):
        ...
        for row in result:
            ...
            message_id = "<%s@%s>" % (row[0], group_name)
            line_count = len(row[6].split('\n'))
            overviews.append("%s\t%s\t%s\t%s\t%s\t%s\t%s" % (row[0], mime_encode_header(row[4]), mime_encode_header(author), formatted_time, message_id, row[1], len(row[6]), line_count))
        return "\r\n".join(["%s" % k for k in overviews])
